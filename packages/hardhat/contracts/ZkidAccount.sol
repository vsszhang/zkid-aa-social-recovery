...
contract ZkidAccount is SimpleAccount {
    ...
    mapping(uint256 => mapping(address => bool)) isGuardin;

    // proposal num => guardian => verifier => uint8 (0 is false, 1 is true)
    mapping(uint256 => mapping(address => mapping(address => uint8))) isQualified;

    // proposal num => guardian => uint8 (aggreeOrNot, 0 is false, 1 is true)
    mapping(uint256 => mapping(address => uint8)) aggreeOrNot;
    ...
    constructor(IEntryPoint entryPoint) SampleAccount(entryPoint) {}
    ...
    function verifierRegistrate(address[] verifiers) public onlyOwner {
    ...
    function verifierRemove() public onlyOwner {
        oldVerifier0 = VERIFIER0;
        oldVerifier1 = VERIFIER1;
        oldVerifier2 = VERIFIER2;
    ...
    function submitRecovery(
        uint256 proposalNum,
        bool _aggreeOrNot
    ) public onlyQualifiedGuardian {
        require(isGuardin[msg.sender] == true, "u are not gardian");
        if (aggreeOrNot) {
    ...
    function reset(uint256 proposalNum, address[] guardians) internal {
    ...
    modifier onlyQualifiedGuardian() {
        uint8 res = isQualified[VERIFIER0][msg.sender] +
            isQualified[VERIFIER1][msg.sender] +
            isQualified[VERIFIER2][msg.sender];
